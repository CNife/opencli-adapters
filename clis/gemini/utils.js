...
function readGeminiSnapshotScript() {
    ...
    (() => {
      ...
      const isButtonMatch = (value, patterns) => patterns.some((p) => typeof p === 'string' ? value === p : p.test(value));
      const stopTextPatterns = [/^stop/, /^停止/, /^中止/, /^pause/i];
      const stopAriaPatterns = [/stop/i, /停止/, /中止/, /pause/i];
      const stopBtn = Array.from(document.querySelectorAll('button, [role="button"]')).find((node) => {
        const text = (node.textContent || '').trim().toLowerCase();
        const aria = (node.getAttribute('aria-label') || '').trim().toLowerCase();
        return isButtonMatch(text, stopTextPatterns) || isButtonMatch(aria, stopAriaPatterns);
      });
      // Also detect generating state by checking the send button being disabled
      // (Angular Gemini disables the send button during generation)
      const sendingIndicator = !!Array.from(document.querySelectorAll('button')).find((node) => {
        const aria = (node.getAttribute('aria-label') || '').trim().toLowerCase();
        const disabled = node.disabled || node.getAttribute('aria-disabled') === 'true';
        return disabled && (aria === '发送' || aria === 'send');
      });
      const isGenerating = !!stopBtn || sendingIndicator;
...
function getTranscriptLinesScript() {
    ...
    (() => {
      ...
      const removableSelectors = [
        ...
        'audio-overview-discovery-dialog',
      ];
